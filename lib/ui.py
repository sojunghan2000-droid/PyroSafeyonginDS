...
_HELP_FAQ = [
    ...
    ("화기작업구간 점검은 어떻게 진행하나요? (v1.6)",
     "**5단계 흐름** (일일/수시):\n"
     "(1) **3. 안전점검** → **[+ 신규 점검 회차]** → 점검 주기 **'일일 점검'** 선택 → "
     "자유 등록 모드 자동 활성화(체크 해제 가능) → 저장\n"
     "(2) 회차 **[점검]** → **[+ Task 추가]** → 4번째 탭 **🔥 화기작업 구간** → "
     "**층 + 작업 구간(자유 텍스트) + 업체** 입력 후 등록\n"
     "(3) Task 행 **[점검 시작 →]** → 점검 종류에서 **'화기작업구간 점검'** 선택 → 양호/불량 결정\n"
     "(4) **불량 시** — 사유 multiselect (6종: 방화포 미비치/파손, 소화기 부족·충전·고장, "
    ...
    ("가설컨테이너 사무실 점검은? (v1.6)",
     ...
     "(2) **불량 시** — 사유 multiselect (7종: 소화기 비치·점검 불량, 환기팬 설치·작동, "
...
def _help_dialog() -> None:
    st.markdown(
        ...
        "자주 묻는 질문 16개. 추가 안내는 안전 관리자에게 문의하세요."
        "</div>",
