...
_HELP_FAQ = [
    ("알림 벨의 숫자는 무엇인가요?",
     "조치 대기 지적사항 수 + 지연 회차 수의 합입니다. 클릭하면 popover에서 "
     "각 항목 카드를 보고 클릭으로 해당 페이지(작업 조치 관리 또는 안전점검)로 이동합니다."),
    ("점검은 어떻게 시작하나요? (v1.5)",
     "(1) 안전점검 → 회차 [상세] → 각 Task의 [점검 시작 →] 버튼 → 모바일 친화 점검 모달.\n"
     "(2) 대시보드의 [📷 점검 (QR 스캔)] 버튼 → 작업 유형 선택 → QR 스캔.\n"
     "(3) 장비에 부착된 QR 스티커를 휴대폰으로 직접 스캔 → 자동 진입."),
    ("점검 회차(Round)는 무엇인가요?",
     "v1.5부터 신규 일정 등록 시 1개 회차 + 그 안에 점검 작업(TSK) N개가 한 번에 "
     "생성됩니다. 회차의 점검 주기는 4종(주간/월간/분기/연간)이며, 시설 종류와 "
     "직교라 한 회차에 여러 시설을 포함할 수 있습니다. 별지5 양식의 법정 점검종류 "
     "(임시소방시설/피난로/화기취급감독)는 점검 행 단위로 별도 체크합니다. "
     "회차 단위로 진행률·상태가 자동 집계되며, 회차 단위 별지5 PDF 출력도 가능합니다. "
     "'대상 미선택 (자유 점검)' 옵션으로 회차만 만든 뒤 점검자가 [+ Task 추가]로 "
     "동적 등록도 가능합니다."),
    ("QR 코드는 어디서 발급하나요?",
     "2. 시설 관리 → 각 장비 행의 [QR] 버튼 → 모달에서 PNG 다운로드. "
     "여러 장비를 한 번에 출력하려면 상단의 [QR 스티커 일괄 출력] (A4 4×6 시트)."),
    ("불량 점검 결과의 후속 조치는 어떻게 하나요?",
     "(1) 점검 시작 모달에서 '현장에서 즉시 조치 완료'를 체크하면 즉시 별지5에 "
     "조치 단계까지 기록됩니다.\n"
     "(2) 미조치는 4. 작업 조치 관리 페이지의 [조치 입력 →] 버튼으로 별도 시점에 "
     "조치 결과(내용/사진/확인자)를 입력합니다."),
    ("PDF는 어디서 출력하나요?",
     "(1) 5. 보고서 → 별지5/6/9 전체 PDF. (2) 안전점검 회차 [상세] → "
     "[별지5 PDF (N건)] — 그 회차만의 회차 단위 지적내역서."),
    ("위치(spot)는 어떻게 정의하나요?",
